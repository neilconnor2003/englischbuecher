...
// address already used elsewhere in this project if it's not set.
async function sendAdminOrderNotification({ transporter, db, order, user, orderId }) {
  const fromAddress = `"EnglischBücher Orders" <${process.env.SMTP_USER}>`;
  let toEmail, subject, html;
  try {
    toEmail = process.env.ADMIN_NOTIFICATION_EMAIL || 'neilconnor2003@gmail.com';
    const items = Array.isArray(order.order_items) ? order.order_items : [];
    const addr = order.shipping_address || {};
    const customerName = user
      ? `${user.first_name || ''} ${user.last_name || ''}`.trim() || user.email
      : 'Guest';
    const customerEmail = user?.email || order.paymentResult?.email_address || 'N/A';

    const itemsHtml = items.map(it => `
      <tr>
        <td style="padding:6px 8px;border-bottom:1px solid #eee;">${it.title_en || it.title_de || 'Book #' + it.bookId}</td>
        <td style="padding:6px 8px;border-bottom:1px solid #eee;text-align:center;">${it.quantity}</td>
        <td style="padding:6px 8px;border-bottom:1px solid #eee;text-align:right;">€${Number(it.price || 0).toFixed(2)}</td>
      </tr>
    `).join('');

    html = `
      <div style="font-family:-apple-system,'Segoe UI',sans-serif;max-width:560px;margin:0 auto;">
        <div style="background:linear-gradient(135deg,#1f1633,#3b1d6e);padding:28px 32px;border-radius:12px 12px 0 0;">
          <div style="color:#c4b5fd;font-size:12px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;">EnglischBücher — New Order</div>
          <h1 style="color:#fff;font-size:20px;margin:6px 0 0;">Order #${orderId}</h1>
        </div>
        <div style="padding:24px 32px;border:1px solid #ede9fe;border-top:none;border-radius:0 0 12px 12px;">
          <p style="margin:0 0 6px;"><strong>Customer:</strong> ${customerName}</p>
          <p style="margin:0 0 6px;"><strong>Email:</strong> ${customerEmail}</p>
          <p style="margin:0 0 16px;"><strong>Total:</strong> €${Number(order.total || 0).toFixed(2)} &nbsp; | &nbsp; <strong>Paid:</strong> ${order.is_paid ? 'Yes' : 'No'}</p>

          <p style="margin:0 0 4px;font-weight:700;">Shipping address</p>
          <p style="margin:0 0 16px;color:#444;">${addr.address || '-'}, ${addr.city || ''} ${addr.postalCode || ''}, ${addr.country || ''}</p>
          ...
          <div style="text-align:center;margin-top:24px;">
            <a href="${process.env.FRONTEND_URL || 'https://englischbuecher.de'}/admin/orders"
               style="display:inline-block;background:#7C3AED;color:#fff;padding:12px 26px;border-radius:999px;text-decoration:none;font-weight:700;font-size:14px;">
    ...
    `;
    subject = `🛒 New order #${orderId} — €${Number(order.total || 0).toFixed(2)}`;
    ...
    if (db) {
      await db.execute(`
        ...
        VALUES (?, ?, ?, ?, 'sent', 'AdminOrderNotification', NOW())
      `, [toEmail, fromAddress, subject, html]).catch(() => {});
    }
  } catch (err) {
    // Never let a notification failure affect order creation
    console.error('[sendAdminOrderNotification] failed:', err.message);
    if (db) {
      await db.execute(`
        INSERT INTO sent_emails (to_email, from_email, subject, html, status, error, type, created_at)
        VALUES (?, ?, ?, ?, 'failed', ?, 'AdminOrderNotification', NOW())
      `, [toEmail || null, fromAddress, subject || `New order #${orderId}`, html || null, err.message]).catch(() => {});
    }
  }
}
...
module.exports = (db, transporter) => {
  ...
  async function sendReviewRequestEmailNow({ db, transporter, toEmail, firstName, language, book, orderId, bookId, userId }) {
    ...
    // Was previously just `process.env.SMTP_USER` with no display name,
    // which is why this email was showing up as sent by "admin" instead
    // of "EnglischBücher" — every other email in the app uses this same
    // branded from string, this one was just missed.
    ...
    try {
      ...
      `, [toEmail, fromAddress, subject, html, err.message]).catch(() => {});
    ...
    try {
      ...
      if (userId) {
        ...
        if (orderUser) {
          ...
          // fire and forget
          sendInvoiceEmail(fullOrder, orderUser, orderUser.language || 'de').catch(console.error);
        }
      }

      // 8) Notify the shop owner — fire and forget, never blocks the response
      sendAdminOrderNotification({ transporter, db, order: fullOrder, user: orderUser, orderId });
  ...
  router.post('/finalize-from-payment-intent', requireAuth, async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        // fullOrder is shared by both the customer invoice and the admin
        // notification below — this is the object sendInvoiceEmail.js expects.
        const finalizeFullOrder = {
          id: orderId,
          user_id: userId,
          order_items: orderItems,
          shipping_address: shippingAddress || {},
        ...
        if (userId) {
          try {
            const [[u]] = await db.execute('SELECT * FROM users WHERE id = ?', [userId]);
            finalizeUser = u || null;
          } catch (_) { }
        }

        // Customer-facing invoice email — this was the missing piece on this
        // path. Redirect-based payment methods (PayPal, Klarna, giropay, etc.)
        // land here instead of the main POST '/' route, which is the only
        // place that was sending it, so orders paid this way never got an
        // invoice email or a log entry at all. Fire and forget, never blocks
        // the response.
        if (finalizeUser) {
          const sendInvoiceEmail = require('../utils/sendInvoiceEmail');
          sendInvoiceEmail(finalizeFullOrder, finalizeUser, finalizeUser.language || 'de').catch(console.error);
        }

        // Notify the shop owner — fire and forget, never blocks the response
        sendAdminOrderNotification({
          transporter,
          db,
          order: finalizeFullOrder,
          user: finalizeUser,
          orderId,
        });
